...
assert.match(viewerRuntime, /roadgen3d-asset-placement-highlights/);
assert.match(viewerRuntime, /viewer-asset-placement-hint/);
assert.match(viewerRuntime, /function startAssetPlacement/);
assert.match(viewerRuntime, /function placeAssetAtCurrentTarget/);
assert.match(viewerRuntime, /showAssetPlacementHighlights/);
assert.match(viewerRuntime, /placeAssetAtCurrentTarget\(event\.altKey\)/);
assert.match(viewerRuntime, /support_policy: placement\.valid \? "strict" : "manual_override"/);
assert.match(viewerRuntime, /按住 Alt 点击可人工覆盖/);
assert.match(api, /support_policy\?: "strict" \| "manual_override"/);
assert.match(styles, /viewer-asset-placement-hint/);
